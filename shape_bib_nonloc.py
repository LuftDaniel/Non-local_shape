...
class NS_MeshData:
    ...
    # Objekt mit den Datenarrays des Gitters passend zum non-local solver
    def __init__(self, meshdata, bounds):

        if(isinstance(meshdata, MeshData)): pass
        else: raise SystemExit("Benoetigt Objekt der MeshData-Klasse!")

        # Mesh Knoten mit Koordinaten: Label stimmt mit Index des Arrays ueberein
        self.verts = meshdata.mesh.coordinates()

        # Daten der Triangulierung: 1 Eintrag Subdomain, 2,3,4 Indizes der zugehoerigen Knoten
        #                           Label stimmt mit Index des Arrays ueberein
        cells = meshdata.mesh.cells()
        subdoms = meshdata.subdomains.array()
        self.triangles = np.column_stack((subdoms, cells))

        # Daten des inneren Randes (Interface): Indizes der zugehoerigen Knoten
        # ACHTUNG WEGEN BOUNDARIES UND MESH INDIZES, DENN INTERACTION KOMMT DAZU
