from fenics import *
import shape_bib_nonloc as bib
import time
import argparse
import os
import numpy as np
...
file_sol_y       = File(os.path.join(outputfolder,
                                     'Last_Step_Data',   'sol_y.pvd'))
file_adj_p       = File(os.path.join(outputfolder,
                                     'Last_Step_Data', 'adj_p.pvd'))
...
# Berechne Indizes des Randes des Einheitsquadrats des Zielgitters und des Startgitters
# Diese werden einmal berechnet, da diese invariant sind

ind_boundary_facets = []
for i in range(0,len(targetMeshData.boundaries)):
...
ind_boundary_vertices_target = []
for c in cells(targetMeshData.mesh):
    for f in facets(c):
        if f.index() in ind_boundary_facets:
            for v in vertices(f):
                ind_boundary_vertices_target.append(v.index())
...
target_NS_boundaries = list(set(ind_boundary_vertices_target))
NS_boundaries        = list(set(ind_boundary_vertices))

# Gitterdaten konvertieren
NS_targetMeshData = bib.NS_MeshData(targetMeshData, target_NS_boundaries)
...
#
# # Daten in FEniCS uebersetzen
# V_target = FunctionSpace(targetMeshData.mesh, "P", 1)
# y_z = Function(V_target)
#
# # Indizes der Vertizes zu DOF Vertizes permutieren
# vtod = dolfin.vertex_to_dof_map(V_target)
# DOM_OUTPUT_Y_Z = np.zeros(len(targetMeshData.mesh.coordinates()))
#
# for i in range(0, len(targetMeshData.mesh.coordinates())):
#       DOM_OUTPUT_Y_Z[vtod(i)] = OUTPUT_Y_Z[i]
#
# y_z.vector()[:] = DOM_OUTPUT_Y_Z
...
while nrm_f_elas[1] > tol_shopt:
    ...
    # Gitterdaten konvertieren
    NS_MeshData = bib.NS_MeshData(MeshData, NS_boundaries)
    ...
    # Non-loc. solver
        ...
        # Zieldaten in Solverform konvertieren
    # V = FunctionSpace(MeshData.mesh, "P", 1)
    # u_bar = np.zeros(len(MeshData.mesh.coordinates()))
    # vtod = dolfin.vertex_to_dof_map(V)
    # for i in range(0, len(u_bar)):
    #     u_bar[i] = z.vector().get_local()[vtod(i)]

    # u ist schon in der solverform! solve
    ...
    #
    # # Indizes der Vertizes zu DOF Vertizes permutieren
    # DOM_OUTPUT_Y = np.zeros(len(MeshData.mesh.coordinates()))
    # DOM_OUTPUT_P = np.zeros(len(MeshData.mesh.coordinates()))
    # for i in range(0, len(MeshData.mesh.coordinates())):
    #       DOM_OUTPUT_Y[vtod(i)] = OUTPUT_Y[i]
    #       DOM_OUTPUT_P[vtod(i)] = OUTPUT_P[i]
    # y.vector()[:] = DOM_OUTPUT_Y
    # p.vector()[:] = DOM_OUTPUT_P
